...
Replace `pmadhavireddy` with your Docker Hub username.

### Frontend
```bash
docker build -t pmadhavireddy/novacart-frontend:v1 ./frontend
docker tag pmadhavireddy/novacart-frontend:v1 pmadhavireddy/novacart-frontend:latest
docker push pmadhavireddy/novacart-frontend:v1
docker push pmadhavireddy/novacart-frontend:latest
```

### Product Service
```bash
docker build -t pmadhavireddy/novacart-product-service:v1 ./product-service
docker tag pmadhavireddy/novacart-product-service:v1 pmadhavireddy/novacart-product-service:latest
docker push pmadhavireddy/novacart-product-service:v1
docker push pmadhavireddy/novacart-product-service:latest
```

### Cart Service
```bash
docker build -t pmadhavireddy/novacart-cart-service:v1 ./cart-service
docker tag pmadhavireddy/novacart-cart-service:v1 pmadhavireddy/novacart-cart-service:latest
docker push pmadhavireddy/novacart-cart-service:v1
docker push pmadhavireddy/novacart-cart-service:latest
```
...
Before applying manifests, update image names in `k8s/*.yaml` from `pmadhavireddy/...` to your Docker Hub username.
